...
- An older integration example exists at https://github.com/Lucra-Sports/lucra-demo-product — useful for event-wiring patterns; its `new LucraClient(...)` constructor is the pre-1.0 API, don't copy it. Use `LucraClient.initialize(...)` per current docs.
...
6. **Round-trip**: `client.api.tournaments()` works pre-auth and proves the postMessage channel end-to-end. Note: `api.*` requests are single-flight — issuing a new call cancels the in-flight one, which rejects with `"Cancelled by new <name> request"`. React StrictMode's dev-mode double effects trigger this constantly; treat that specific rejection as benign, not an error.
